...
## 40. A rate that finds out what the client will take — DROPPED 2026-09-13

**Dropped by Alberto, 2026-09-13**: *40. - eliminare*. The entry asked whether Wide Family could
send faster than ten messages a second by raising the rate while the client accepted every
hand-off. Its text is removed; the heading stays because backlog 49 and a 2026-09-08 decision
still point here. The rate stays `PER_TICK = 2` every `TICK = 0.2` in `Family/Comm.lua`.
...
**Built 2026-09-15.** One switch under Options, *Fold long lists to fit the page*, ticked unless
...
## 85. An Options switch: Possessions draws the bags as one block, and the bank as one block — DONE 2026-09-15, seen working in game
...
**Still open**: the low-skill shape - a miner at 50 over a vein wanting 275 - which nobody here
has read, and which may replace the profession line as a non-gatherer's does or may sit under it.
Both are covered and neither is established; and the in-game confirmation of the mining half, which should be taken on the rows the
