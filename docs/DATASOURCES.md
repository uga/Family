...
> **Corrected 2026-09-12: that paragraph was measured on smelting and is wrong for enchanting.** On
> Burning Crusade an enchanting recipe that makes a thing is recorded from the Craft frame with its
> spell and **no item** - read from play on Lesser Magic Wand, whose Professions row showed the
> spell and so carried nothing Family says about a wand. And the claim hid a second, larger gap:
> `MadeBy`'s composition on the other two builds reaches no spell at all for **760** craftable
> products on Burning Crusade and **2,483** on Mists - every recipe a trainer teaches - so *Made
> with* and its prices could never appear on them. Counted against the generated tables.

### The one item a recipe spell makes — `RecipeProducts`, generated 2026-09-12

The same `SpellEffect` rows once more, read from the spell's end: effect **24** (CREATE_ITEM),
`EffectItemType` the product, kept where a profession skill line (category 9 or 11) teaches the
spell. Emitted by `tools/recipe-teaches.py` for **all three builds**, from caches already on disk -
wago.tools (§3), nothing new fetched, and the three tables already in the file regenerated byte for
byte before the fourth was added.

| | |
|---|---|
| Recipe spells naming a product | **1,412** Era, **1,974** Burning Crusade, **4,851** Mists |
| Spells creating more than one item | **0** - and any would be left out, having no single product |
| Size | **156 KB** added to `RecipeTeaches.lua` |

**Only recipes that make a thing are in it**, which is what keeps it to wands, rods and oils among
enchanting. An enchant applied to gear has ENCHANT_ITEM and no CREATE_ITEM: on Burning Crusade
**31 of 229** enchanting spells make an item, and *Enchant Bracer - Minor Health* (7418) and *Enchant
Chest - Minor Mana* (7443) are not in the table at all.

Two readers. `Recipes:Product(spell)` lets a recipe row with a spell and no item describe the item.
`Recipes:MadeBy(item)` reads it backwards after the shipped Era table and before the composition,
lowest spell id winning, which closes the 760 and the 2,483.
