...
function skillsOf(meta, secondary)
	...
	for id, skill in pairs(meta.skills or {}) do
		-- **A class skill counts as a secondary here**, which reverses a decision taken
		-- when this list was words. Lockpicking has a rank and a maximum and reads exactly
		-- like a profession from here; it was moved to the abilities panel because it is
		-- technically an ability, and Alberto has asked for it back - the rows are pictures
		-- and a rank now, so a rogue reading their own line wants it where the rest of what
		-- they have is, not on another page. Confirmed rather than assumed, because the
		-- comment that used to sit here recorded him deciding the other way.
		--
		-- It stays out of the *professions panel*, which is about what a member can make and
		-- where lockpicking would announce a window that does not exist.
