...
Family.locales.ruRU = {
	...
	["A star appears on every panel in the strip; click one to make it the panel Family opens on, every time. The summary is starred with the set of columns you are looking at, so a star can mean Activity rather than the whole panel."] = "На каждой панели в полосе появляется звёздочка; нажмите одну, и Family будет открываться там каждый раз. Сводка отмечается вместе с набором столбцов, который вы видите, — так что звёздочка может означать «Активность», а не всю панель.",
	["Open Family here"] = "Открывать Family здесь",
	["|cff9d9d9dFamily opens on this panel, every time.|r"] = "|cff9d9d9dFamily открывается на этой панели каждый раз.|r",
