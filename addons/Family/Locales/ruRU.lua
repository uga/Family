...
Family.locales.ruRU = {
	...
	["|cffffd700What you share with %s|r   |cff888888one profession at a time, and nothing until it is ticked|r"] = "|cffffd700Чем вы делитесь с %s|r   |cff888888по одной профессии за раз, и ничем, пока не отмечено|r",
	...
	["   |cffffaa00%d left out: this client gave no identifier for them, and a word is one language, so they cannot be shared.|r"] = "   |cffffaa00Пропущено: %d. Этот клиент не дал для них идентификатора, а слово — это один язык, поэтому поделиться ими нельзя.|r",
