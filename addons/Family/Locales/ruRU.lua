...
Family.locales.ruRU = {
	...
	["|cff888888Crafting cooldowns only - transmutes, mooncloth, salt shakers. A column appears once Family has seen that cooldown running at least once, because the client will not say a recipe has one while it is ready. Blank means never seen it, which is not the same as nought.%s|r"] = "|cff888888Только ремесленные восстановления - трансмутации, луноткань, солонки. Столбец появляется, когда Family хотя бы раз видел это восстановление идущим, потому что клиент не сообщает о нём, пока оно готово. Пусто означает «ни разу не видели», а это не то же самое, что ноль.%s|r",
