...
Family.locales.ruRU = {
	...
	["|cff888888Every skill on one line per member: the two primary professions first, then the secondary ones. Weapon skills go on the line below. A profession in grey has recipes Family has not seen for a week, or has never seen: ranks are always current, recipe lists are only as new as the last time that window was open.|r"] = "|cff888888Все умения в одной строке на персонажа: сначала две основные профессии, затем второстепенные. Владение оружием — строкой ниже. Серая профессия имеет рецепты, которых Family не видел неделю или не видел вовсе: ранги всегда актуальны, а списки рецептов настолько свежи, насколько давно открывалось то окно.|r",
