...
Family.locales.ruRU = {
	...
	["   |cffffaa00Not offered: %s. This client did not say which profession that is, only what it is called - and a name is readable in one language, so it cannot cross.|r"] = "Не предлагается: %s. Этот клиент не сказал, что это за профессия, — только как она называется, а имя читается лишь на одном языке, поэтому передать его нельзя.",
