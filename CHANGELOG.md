...
**And it has to be short enough to finish.** A release note is read on a project page by
somebody deciding whether to install this, usually on a phone, usually for about twenty
seconds. So: a sentence at the top saying what the release is for; headings a reader can skim
that name the thing itself — *profession cooldowns*, *reputations*, *mail*, *mounts* — rather
than *Added* and *Fixed*; one sentence a bullet, benefit first; the biggest change first, not
whichever landed first. Related changes are one entry, not eight. Twenty entries a release is
plenty and fifty is a wall nobody reads, which fails the same test a vague page does.

Those headings are also how anybody finds this addon at all: somebody searching for an alt
manager types *profession cooldown*, *bank alt*, *who can make this* — the words the game
uses, in the heading, where a search engine and a skimming reader both look first.
...
- **`/family decodecost`** says how long it takes this client to unpack every character's record,
  the slowest three, and how many recipe lists are in another language. A measurement for a slowdown
  being looked into; it changes nothing.
...
**Two crafting cooldowns that were telling you the wrong thing.**

### Crafting cooldowns

- **A salt shaker that said it was ready in 49 days.** Its cooldown is three. Family no longer
  believes an answer that says a cooldown has longer left than it lasts, and the next scan
  records the real one.

- **A mining cooldown that does not exist.** A Gold Bar is smelted by a miner and transmuted by
  an alchemist, and only the alchemist waits — so any miner who could smelt gold grew a Mining
  column stuck on *ready* for ever. The wrong mark clears itself the next time that profession's
  window is opened.
